...
export async function mockAnthropicDebrief(page: Page): Promise<void> {
  await page.route('**/api/debrief/generate', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        data: {
          ...
          insights: {
            ...
            weaknesses: [
              { badge: 'Critique', text: 'Jeudi après 15h : 4 pertes sur 5' },
            ],
            emotionInsight: 'Corrélation forte entre STRESSED et pertes.',
            objectives: [
              {
                title: 'Stopper après 15h le jeudi',
          ...
          objectives: [
            { title: 'Stopper après 15h le jeudi', reason: 'Pattern détecté' },
          ],
